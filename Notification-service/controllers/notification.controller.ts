...
import { getSingleNotification } from "../services/notifications.service";
...
export const deleteNotification = async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    await notificationQueue({ action:"delete", notificationId, userId, status: "DELETED" });
...
export const deleteAllNotifications = async (req: any, res: Response, next: NextFunction) => {
  try {
    const userId = req.user.id;
    ...
    await notificationQueue({ action:"delete-all", userId, status: "DELETED" });
...
export const getSingleNotificationById = async (req: any, res: Response, next: NextFunction) => {
  try {
    const { notificationId } = req.params;
    const userId = req.user.id;
